...
export const UsersFilteredButtons: React.FC<PropsType> = (props) => {
    ...
    return (
        <div className={styles.root}>
            <button
                onClick={() => onClickFilterChangedHandler({ friend: null, term: filter.term })}
                className={`${styles.button} ${filter.friend === null ? styles.buttonSelected : ""}`}
            >
                All
            </button>
            <button
                onClick={() => onClickFilterChangedHandler({ friend: true, term: filter.term })}
                className={`${styles.button} ${filter.friend === true ? styles.buttonSelected : ""}`}
            >
                Followed
            </button>
            <button
                onClick={() => onClickFilterChangedHandler({ friend: false, term: filter.term })}
